  ...
  return (
    <div className="min-h-screen bg-[#0B0B1A] pb-20 overflow-x-hidden">
      ...
      <div className="px-4 -mt-16 flex flex-col gap-6 mb-24">
        {/* Stats */}
        <div className="bg-[#1C1C2E] rounded-[2.5rem] p-6 border border-[#30363d] shadow-lg relative overflow-hidden">
          ...
          {filtered.map(friend => (
            <div key={friend.id} className="bg-[#1C1C2E] p-5 rounded-[2rem] border border-[#30363d] hover:border-purple-500/30 transition-all cursor-pointer group" onClick={() => navigate(`/profile/${friend.username}`)}>
              ...
              <div className="grid grid-cols-2 gap-3" onClick={e => e.stopPropagation()}>
                ...
                <button onClick={() => handleUnfollow(friend.id, friend.name)} className="bg-[#161b22] text-gray-400 font-bold py-3 rounded-xl hover:bg-red-900/20 hover:text-red-400 transition-all text-xs border border-[#30363d]">
                  Unfollow
                </button>
              </div>
            </div>
