...
const Winners: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0B0B1A] font-inter text-white pb-24 overflow-x-hidden">
      <header className="relative overflow-hidden bg-gradient-to-br from-[#110a2a] via-[#1d0d4a] to-[#0d1a6b] p-4 sm:p-6 pt-12 pb-24 text-center shadow-lg shadow-purple-900/30">
        ...
        <div className="relative z-10">
          <h1 className="text-sm font-black uppercase tracking-[0.5em] text-amber-400 mb-2">Hall of Fame</h1>
      ...
      <div className="px-5 space-y-6 mt-12">
        ...
        {winners.map((w, i) => (
          <motion.div key={w.id || i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
            className="bg-[#1C1C2E] border border-white/5 p-4 sm:p-6 rounded-[3rem] flex flex-wrap items-center gap-6 relative group shadow-md hover:border-purple-500/30 transition-all">
            <div className="relative">
              ...
              <div className="absolute -top-2 -right-2 w-8 h-8 bg-amber-500 rounded-full flex items-center justify-center text-slate-950 font-black text-sm z-20 shadow-xl">
                #{i + 1}
              </div>
            </div>
            <div className="flex-1">
              <h3 className="text-xl font-black tracking-tighter mb-1 text-white">{w.username || 'Anonymous'}</h3>
              <p className="text-sm font-black text-amber-400 uppercase tracking-widest mb-3">{w.prize_won || 'Prize Won'}</p>
              <div className="flex flex-wrap items-center gap-2 text-xs sm:text-sm font-bold text-white/40 uppercase">
                <span>Won:</span>
                <span className="text-white text-sm font-black">{w.prize_won || 'N/A'}</span>
              </div>
            </div>
