...
const BBCodeEditor: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0B0B1A] pb-32 font-inter flex flex-col overflow-x-hidden">
      <header className="relative overflow-hidden bg-gradient-to-br from-[#110a2a] via-[#1d0d4a] to-[#0d1a6b] text-white p-4 sm:p-6 pb-20 rounded-b-[3.5rem] shadow-xl shrink-0">
        ...
        <div className="relative z-10 flex items-center gap-4 flex-wrap">
          ...
          <div>
            ...
            <p className="text-sm font-black uppercase tracking-[0.2em] text-white/50">Creative Workspace</p>
          </div>
        </div>
      </header>

      <div className="px-5 -mt-10 flex-1 space-y-6">
        <div className="bg-[#1C1C2E] rounded-[2.5rem] border border-white/5 shadow-lg flex flex-col">
          {/* Title input */}
          ...
          {/* Toolbar */}
          {!previewMode && (
            <div className="px-3 sm:px-6 py-3 border-b border-[#30363d] flex flex-wrap gap-2 overflow-x-auto no-scrollbar">
            ...
            <AnimatePresence mode="wait">
              {previewMode ? (
                <motion.div key="preview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  className="p-4 sm:p-6 text-[13px] text-gray-300 leading-relaxed min-h-full">
                ...
                <motion.textarea key="editor" ref={textareaRef} value={content} onChange={e => setContent(e.target.value)}
                  ...
                  className="w-full h-full p-4 sm:p-6 text-[14px] font-medium text-gray-200 bg-transparent border-none outline-none resize-none" />
              )}
            </AnimatePresence>
          </div>

          {/* Bottom actions */}
          <div className="p-4 sm:p-6 bg-[#161b22] border-t border-white/5 flex items-center justify-between">
            <button onClick={() => navigate('/bb-guide')}
              className="text-sm font-black text-white/40 uppercase tracking-widest hover:text-purple-400 transition-colors">📖 BBCode Guide</button>
            <button onClick={handlePublish} disabled={!content.trim() || publishing}
              className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-4 sm:px-8 py-3.5 rounded-2xl shadow-xl shadow-purple-900/30 font-black text-xs sm:text-sm uppercase tracking-widest active:scale-95 transition-all disabled:opacity-50 hover:opacity-90">
