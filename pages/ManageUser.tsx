...
const AdminUserManagement: React.FC = () => {
  ...
  // Tab State
  const [activeTab, setActiveTab] = useState<'dashboard' | 'tools' | 'audit'>('tools');
  ...
  // Mock Admin (Since RBAC auth isn't in scope of this file)
  const adminId = 'admin_session';
  const adminName = 'System Moderator';
  ...
  useEffect(() => {
    if (activeTab === 'audit' && userId) {
      mongoService.getUserAuditLogs(userId).then(setAuditLogs);
    }
  }, [activeTab, userId]);
  ...
  const handleAdminUpdate = async (updates: any) => {
    ...
    const res = await mongoService.adminUpdateUser(targetUser.id, updates, reason, adminId, adminName);
    if (res) {
      setTargetUser(res);
  ...
  const forceLogout = async () => {
    ...
    const success = await mongoService.forceLogoutUser(targetUser.id, reason, adminId, adminName);
    if (success) {
  ...
  return (
    <div className="min-h-screen bg-[#0F0F1A] text-white pb-12 font-inter w-full overflow-x-hidden text-sm sm:text-base">
      ...
      <div className="px-4 -mt-8 space-y-5 relative z-10 max-w-4xl mx-auto w-full">
        ...
        {/* TOP SUB-NAV BAR */}
        <div className="grid grid-cols-3 gap-2 bg-[#1C1C2E] border border-white/5 p-1.5 rounded-2xl relative z-20">
          <button 
            onClick={() => setActiveTab('dashboard')} 
            className={`py-2.5 text-sm sm:text-base font-black rounded-xl text-center transition-all ${activeTab === 'dashboard' ? 'bg-purple-600 text-white shadow-lg' : 'text-white/40 hover:text-white/70 hover:bg-white/5'}`}>
            📊 Dashboard
          </button>
          <button 
            onClick={() => setActiveTab('tools')} 
            className={`py-2.5 text-sm sm:text-base font-black rounded-xl text-center transition-all ${activeTab === 'tools' ? 'bg-purple-600 text-white shadow-lg' : 'text-white/40 hover:text-white/70 hover:bg-white/5'}`}>
            🔧 User Tools
          </button>
          <button 
            onClick={() => setActiveTab('audit')} 
            className={`py-2.5 text-sm sm:text-base font-black rounded-xl text-center transition-all ${activeTab === 'audit' ? 'bg-purple-600 text-white shadow-lg' : 'text-white/40 hover:text-white/70 hover:bg-white/5'}`}>
            📋 Audit Log
          </button>
        </div>

        {/* TARGET USER PROFILE CARD (Always Visible) */}
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'tools' && (
            <motion.div 
              ...
              className="space-y-5"
            >
              {/* ACTION REASON - INPUT (Must be prominent at the top) */}
              <div className="bg-[#1C1C2E] border border-amber-500/30 rounded-2xl p-4 sm:p-5 space-y-3 shadow-lg shadow-amber-900/10">
                ...
                <input
                  ...
                  placeholder="Provide a valid reason for auditing logs (e.g. 'Spamming in shoutbox')..."
              ...
              <div className="bg-[#1C1C2E] border border-white/5 rounded-2xl p-4 sm:p-5 space-y-4">
                ...
                <div className="bg-[#161626] border border-white/5 p-4 sm:p-5 rounded-xl space-y-4">
                  ...
                  <p className="text-xs sm:text-sm text-white/40 font-medium">
                    Hitting 3 strikes automatically triggers a system restriction (Full Ban).
                  </p>
          ...
          {activeTab === 'audit' && (
            <motion.div 
              ...
              exit={{ opacity: 0, y: -10 }}
              transition={{ duration: 0.2 }}
              className="bg-[#1C1C2E] border border-white/5 rounded-2xl p-4 sm:p-5 space-y-4"
            >
              <div className="flex items-center justify-between">
                <h4 className="text-sm sm:text-base font-black text-white/60 uppercase tracking-widest flex items-center gap-2">
                  📋 Administrative Audit Log
                </h4>
                <span className="text-xs bg-white/5 text-white/40 px-3 py-1 rounded-full font-bold">
                  {auditLogs.length} Events Logged
                </span>
              </div>

              {auditLogs.length === 0 ? (
                <div className="bg-[#161626] border border-white/5 rounded-xl p-8 text-center">
                  <p className="text-white/40 font-bold text-sm sm:text-base">No administrative actions have been logged for this user.</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {auditLogs.map((log) => (
                    <div key={log.id} className="bg-[#161626] border border-white/5 rounded-xl p-4 flex flex-col gap-2">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/5 pb-2">
                        <p className="text-sm sm:text-base font-black text-white">
                          {log.action}
                        </p>
                        <p className="text-[11px] sm:text-xs font-bold text-white/40 bg-white/5 px-2 py-1 rounded w-fit">
                          {new Date(log.timestamp).toLocaleString()}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs sm:text-sm font-bold text-white/60 uppercase tracking-wider mb-1">Moderator Reason</p>
                        <p className="text-sm text-white/90 font-medium italic border-l-2 border-amber-500/50 pl-3">
                          "{log.details}"
                        </p>
                      </div>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="w-5 h-5 rounded-full bg-purple-500/20 text-purple-400 flex items-center justify-center text-[10px] font-black">
                          {log.deletedByName ? log.deletedByName.charAt(0) : 'S'}
                        </span>
                        <p className="text-xs font-bold text-white/40">Action taken by: {log.deletedByName || 'System'}</p>
                      </div>
                    </div>
                  ))}
                </div>
