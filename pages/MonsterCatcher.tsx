  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-white font-sans pb-20 overflow-x-hidden">
      ...
      <div className="bg-slate-900 p-4 sticky top-0 z-10 border-b border-slate-800 flex justify-between items-center shadow-lg">
        ...
        <div className="w-9 text-xs text-right text-emerald-400 font-bold">Streak: {dailyStreak}</div>
      </div>
      ...
      {canClaimDaily && (
        <div className="m-4 p-4 bg-gradient-to-r from-emerald-500/20 to-teal-600/20 border border-emerald-500/30 rounded-xl flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-emerald-200/70">{dailyStreak >= 6 ? 'Claim your +1 Golden Ball' : 'Claim your +5 free Color Balls'}</p>
          </div>
      ...
      <div className="flex px-2 py-4 gap-2 overflow-x-auto no-scrollbar border-b border-slate-800">
        {['hunt', 'collection', 'shop', 'fuse', 'arena', 'market'].map(t => (
          <button
            ...
            className={`px-4 py-2 rounded-full font-black text-xs uppercase tracking-wider shrink-0 transition ${activeTab === t ? 'bg-cyan-500 text-white shadow-[0_0_15px_rgba(6,182,212,0.4)]' : 'bg-slate-800 text-slate-400'}`}
          >
            {t}
          </button>
        ))}
      ...
      <div className="p-4">
                  ...
                  <div key={z.name} className={`bg-gradient-to-br ${z.color} border border-slate-700 p-4 rounded-2xl relative overflow-hidden group`}>
                    <div className="flex justify-between items-start mb-2 relative z-10">
                      ...
                      <div className="text-right">
                        <span className="text-xs font-bold text-white/50 block">⏱️ {z.durationText}</span>
                        <span className="text-xs font-bold text-amber-400 block mt-1">⚡ {z.stamina} Stamina</span>
                      </div>
                    </div>
            ...
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-6 rounded-2xl border border-slate-700 text-center">
              ...
              <div className="flex gap-2 justify-center">
                ...
                <button onClick={() => handleBuyBalls(5, false)} className="px-4 py-3 bg-slate-800 border border-slate-600 rounded-xl hover:bg-slate-700 transition">
                  ...
                  <div className="text-xs text-slate-400 font-bold">250 Plusses</div>
                </button>
              </div>
            </div>

            <div className="bg-gradient-to-br from-amber-900/40 to-orange-900/40 p-6 rounded-2xl border border-amber-500/30 text-center mt-4">
              ...
              <button onClick={() => handleBuyBalls(1, true)} className="px-6 py-3 bg-gradient-to-r from-amber-500 to-orange-500 rounded-xl hover:opacity-90 transition w-full shadow-[0_0_15px_rgba(245,158,11,0.3)]">
                <div className="font-black text-white text-lg mb-1">+1 Golden Ball</div>
                <div className="text-xs text-white/80 font-bold">150 Plusses</div>
              </button>
            </div>
          </div>
        )}
        ...
        {activeTab === 'fuse' && (
          <div className="space-y-4">
             <div className="bg-slate-800/50 border border-slate-700 p-4 rounded-xl text-center mb-6">
               ...
               <p className="text-xs text-slate-400">Combine 3 identical <span className="text-gray-300 font-bold">Common</span> monsters to generate 1 random <span className="text-green-400 font-bold">Uncommon</span> monster!</p>
             </div>
        ...
        {activeTab === 'arena' && (
          <div className="space-y-4">
            <div className="bg-gradient-to-br from-red-900/20 to-rose-900/20 border border-red-500/20 p-4 rounded-xl text-center mb-6">
               ...
               <p className="text-xs text-rose-200/70">Deploy up to 3 monsters. Higher rarity = higher power. Winning grants +50 AP!</p>
            </div>

            <div className="mb-4">
              <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Deploy Team ({selectedTeam.length}/3)</h4>
                ...
                {collection.map((m, i) => {
                  ...
                  return (
                    <button 
                      ...
                      className={`px-3 py-2 text-xs font-black rounded-lg border transition ${isSelected ? 'bg-rose-500 text-white border-rose-400 shadow-[0_0_10px_rgba(244,63,94,0.4)]' : 'bg-slate-800 text-slate-400 border-slate-700'}`}
                    >
        ...
        {activeTab === 'market' && (
          <div className="space-y-6">
            ...
            <div className="bg-slate-800 border border-slate-700 p-4 rounded-xl">
              ...
              <div className="flex gap-2">
                ...
                <input 
                  ...
                  placeholder="Price (➕)" 
                  className="w-24 bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-xs font-bold text-white outline-none placeholder:text-slate-500" 
                />
                <button 
                  onClick={() => {
                  ...
                  className="px-4 py-2 bg-emerald-500 text-white font-black text-xs rounded-lg hover:bg-emerald-400 transition"
                >
                  List
                </button>
              </div>
            ...
            {/* Active Listings */}
            <div>
              <h3 className="font-black text-slate-400 uppercase tracking-widest text-xs mb-4">Active Listings</h3>
              <div className="space-y-3">
                {marketListings.length === 0 ? (
                  <p className="text-center text-slate-500 text-xs font-bold py-4">No monsters listed currently.</p>
                ) : (
