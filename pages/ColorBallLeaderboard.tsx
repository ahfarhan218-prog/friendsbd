...
const ColorBallLeaderboard: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0a0a1a] via-[#0f0f2a] to-[#0a0a1a] pb-20">
      <style>{`
        ...
        .lb-card:hover { border-color:rgba(244,114,182,0.15); }
        .custom-scrollbar::-webkit-scrollbar { width: 3px; }
        .custom-scrollbar::-webkit-scrollbar-track { background: transparent; }
        .custom-scrollbar::-webkit-scrollbar-thumb { background: #30363d; border-radius: 10px; }
      `}</style>

      <header className="bg-gradient-to-r from-pink-700 via-fuchsia-600 to-rose-500 text-white p-4 sm:p-6 pb-16 sm:pb-20 rounded-b-[2rem] sm:rounded-b-[3rem] shadow-lg shadow-fuchsia-900/30">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-black italic">Color Ball Leaderboard 🎨</h2>
              <p className="text-xs sm:text-sm opacity-70 font-bold uppercase tracking-wider">Top ball collectors</p>
            </div>
          </div>
        </div>
      </header>
          ...
          {TABS.map(tab => (
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-sm font-black uppercase tracking-wider transition-all ${
                period === tab.key
                  ? 'bg-gradient-to-r from-fuchsia-600 to-pink-500 text-white shadow-lg shadow-fuchsia-900/40 scale-105'
        ...
        {loading ? (
          ...
          <>
            <div className="lb-card p-4 sm:p-6 flex flex-col items-center">
              ...
              <p className="text-xs font-bold text-white/40 uppercase tracking-widest mb-4">{period === 'daily' ? "Today's" : period === 'weekly' ? "This Week's" : 'All Time'} Top Collectors</p>
              <div className="flex items-end justify-center gap-3 sm:gap-6 w-full min-h-[200px]">
                {podium[1] && (
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="flex flex-col items-center flex-1 max-w-[120px]">
                    <div className="relative mb-2">
                      <img src={podium[1].avatar} className="w-14 h-14 sm:w-16 sm:h-16 rounded-full border-2 border-fuchsia-400/30 object-cover shadow-lg" alt="" />
                ...
                {podium[0] && (
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0 }} className="flex flex-col items-center flex-1 max-w-[140px]" style={{ animation: 'float 4s ease-in-out infinite' }}>
                    <div className="relative mb-2">
                      <img src={podium[0].avatar} className="w-16 h-16 sm:w-20 sm:h-20 rounded-full border-2 border-pink-400/50 object-cover shadow-xl shadow-fuchsia-900/30" alt="" />
                ...
                {podium[2] && (
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="flex flex-col items-center flex-1 max-w-[120px]">
                    <div className="relative mb-2">
                      <img src={podium[2].avatar} className="w-14 h-14 sm:w-16 sm:h-16 rounded-full border-2 border-fuchsia-400/20 object-cover shadow-lg" alt="" />
                      <span className="absolute -top-2 -right-2 text-xl">🥉</span>
                    </div>
            ...
            <div className="lb-card p-4">
              <div className="flex items-center gap-2 px-3 mb-4 text-fuchsia-400 font-black uppercase text-xs tracking-widest">
                  ...
                  return (
                    <motion.div
                      ...
                      className={`flex items-center gap-3 p-3 rounded-2xl transition-all ${
                        isSelf
                          ? 'bg-pink-500/10 border border-pink-500/30 shadow-md'
                          : 'bg-[#161b22]/50 border border-transparent hover:bg-[#161b22] hover:border-white/5'
                      }`}
                    >
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center font-black text-xs ${
                        globalIndex <= 10 ? 'bg-pink-500/20 text-pink-400' : 'bg-[#30363d]/50 text-white/40'
                      }`}>
                        {globalIndex}
                      </div>
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="min-w-0">
                          <h4 className="font-bold text-sm text-white truncate">
                            {rank.name} {isSelf && <span className="text-pink-400 text-xs">(You)</span>}
...
export default ColorBallLeaderboard;
