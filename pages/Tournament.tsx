    ...
    <div key={match.id} className="relative py-6 z-10">
      <div className={`rounded-2xl shadow-lg border overflow-hidden w-44 transition-all duration-300 ${isFinal ? 'bg-gradient-to-br from-amber-600/80 to-orange-900/80 border-amber-500/50 backdrop-blur-md' : 'bg-[#090d16]/90 border-[#30363d] backdrop-blur-sm'}`}>
        <div 
          ...
          className={`px-4 py-3 text-xs sm:text-sm font-black border-b border-[#30363d] flex justify-between items-center transition-all cursor-pointer h-12 ${
        ...
        <div 
          ...
          className={`px-4 py-3 text-xs sm:text-sm font-black flex justify-between items-center transition-all cursor-pointer h-12 ${
  ...
  return (
    <div className="min-h-screen bg-transparent pb-32 overflow-x-hidden">
        ...
        <div className="absolute bottom-0 right-0 p-24 bg-pink-500/10 rounded-full blur-2xl -mr-16 pointer-events-none" />
           ...
           <div className="flex flex-wrap items-center gap-4">
             ...
             <div>
               ...
               <p className="text-sm font-black uppercase tracking-widest text-slate-400">Competitive Arena</p>
             </div>
           </div>
      ...
      <div className="px-5 -mt-16 flex flex-col gap-6 mb-24 relative z-10 max-w-lg mx-auto">
        {/* Promo Card */}
        <div className="bg-gradient-to-br from-purple-700 to-pink-800 rounded-[3rem] p-4 sm:p-8 shadow-[0_0_30px_rgba(192,38,211,0.3)] text-center relative overflow-hidden border border-pink-500/30">
           <div className="absolute top-4 right-1/2 translate-x-1/2 flex flex-wrap items-center gap-2 bg-red-600/80 backdrop-blur-md px-4 py-1.5 rounded-full text-sm font-black uppercase tracking-widest border border-red-400/50 shadow-[0_0_15px_rgba(220,38,38,0.5)]">
           ...
           <div className="mt-6 relative z-10">
              ...
              <p className="text-xs sm:text-sm text-pink-100 font-medium leading-relaxed max-w-[240px] mx-auto mt-4 px-4 bg-black/20 rounded-2xl py-3 border border-white/10 backdrop-blur-sm">
        ...
        <div className="bg-[#161b22]/80 backdrop-blur-xl p-2 rounded-[2rem] shadow-xl border border-[#30363d] grid grid-cols-1 sm:grid-cols-1 sm:grid-cols-2 gap-4 sm:grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-2">
           {(['overview', 'schedule', 'standings', 'bracket'] as const).map((t) => (
             <button
               key={t}
               onClick={() => setTab(t)}
               className={`py-3 rounded-[1.5rem] text-sm font-black transition-all uppercase tracking-widest ${
                 tab === t 
        ...
        {/* Bracket Content */}
        {tab === 'bracket' && (
          <div className="bg-[#161b22]/80 backdrop-blur-xl rounded-[3rem] p-4 sm:p-6 pt-10 shadow-xl border border-[#30363d] overflow-x-auto no-scrollbar relative min-h-[600px]">
             <div className="flex flex-wrap items-center gap-3 text-purple-400 font-black text-xs sm:text-sm uppercase tracking-[0.2em] mb-12 bg-purple-900/30 w-fit px-5 py-2 rounded-full border border-purple-500/20 shadow-[0_0_15px_rgba(168,85,247,0.15)]">
             ...
             <div className="flex flex-wrap gap-12 min-w-[700px] py-4 relative">
                ...
                <div className="flex flex-col justify-between gap-4 relative">
                   <p className="text-sm font-black text-purple-400 uppercase tracking-[0.2em] text-center mb-6 bg-[#0f172a] py-2 rounded-full border border-purple-500/20 shadow-inner">Quarter Finals</p>
                ...
                <div className="flex flex-col justify-around relative">
                   <p className="text-sm font-black text-purple-400 uppercase tracking-[0.2em] text-center mb-6 bg-[#0f172a] py-2 rounded-full border border-purple-500/20 shadow-inner">Semi Finals</p>
                ...
                <div className="flex flex-col justify-center relative">
                   <p className="text-sm font-black text-amber-400 uppercase tracking-[0.2em] text-center mb-6 bg-[#1e1b4b] py-2 rounded-full border border-amber-500/30 shadow-[0_0_15px_rgba(245,158,11,0.2)]">Grand Finale</p>
                      ...
                      <div key={m.id} className="relative">
                        ...
                        {m.winner && (
                          <div className="absolute -top-24 left-1/2 -translate-x-1/2 flex flex-col items-center animate-bounce z-20">
                             ...
                             <span className="text-sm font-black text-amber-200 uppercase mt-3 tracking-[0.2em] bg-amber-900/80 backdrop-blur-sm px-4 py-1.5 rounded-full border border-amber-500/50 shadow-lg">Season Champion</span>
                          </div>
                        )}
                        ...
                        {m.winner && (
                          <div className="absolute -bottom-12 left-0 right-0 text-center">
                             <p className="text-xs sm:text-sm font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-200 to-amber-500 uppercase tracking-widest drop-shadow-md">{m.winner} Wins!</p>
                          </div>
                        )}
                      </div>
                    ))}
             ...
             <div className="mt-20 p-4 sm:p-6 bg-[#0f172a] rounded-[2rem] border border-[#30363d] flex flex-wrap items-center gap-5 shadow-inner">
               ...
               <p className="text-xs sm:text-sm text-slate-400 font-medium leading-relaxed italic pr-2">
        ...
        {/* Other Tabs content */}
        {tab === 'schedule' && (
          <div className="bg-[#161b22]/80 backdrop-blur-xl rounded-[3rem] p-4 sm:p-8 shadow-xl border border-[#30363d] space-y-6">
             <div className="flex flex-wrap items-center gap-3 text-purple-400 font-black text-xs sm:text-sm uppercase tracking-[0.2em] mb-6 bg-purple-900/30 w-fit px-5 py-2 rounded-full border border-purple-500/20 shadow-[0_0_15px_rgba(168,85,247,0.15)]">
             ...
             {[1, 2, 3].map(i => (
               <div key={i} className="bg-[#090d16]/80 p-5 rounded-[2rem] border border-[#30363d] shadow-inner flex items-center justify-between group hover:border-purple-500/50 transition-colors">
                 <div>
                   <p className="text-sm font-black text-purple-400 uppercase tracking-widest mb-1">Match #{i+5}</p>
                   <p className="text-sm font-black text-white italic tracking-tight">TBD vs TBD</p>
                   <p className="text-sm text-slate-500 font-bold mt-2 uppercase tracking-widest flex flex-wrap items-center gap-1">
                      <span className="w-1.5 h-1.5 bg-slate-500 rounded-full inline-block"></span> Stadium Dhaka • Jan 2{i+5}, 2026
                   </p>
                 </div>
                 <div className="bg-purple-900/20 px-4 py-2 rounded-2xl text-sm font-black text-purple-300 border border-purple-500/20 uppercase tracking-[0.2em] shadow-sm">Upcoming</div>
               </div>
             ))}
          </div>
        )}
          ...
          <div className="bg-[#161b22]/80 backdrop-blur-xl rounded-[3rem] p-4 sm:p-8 shadow-xl border border-[#30363d]">
             <div className="space-y-6">
                <div className="flex flex-wrap items-center gap-4 mb-4 bg-[#090d16]/80 p-5 rounded-[2rem] border border-[#30363d]">
                   ...
                   <div>
                      <h4 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-1">Prize Pool</h4>
                ...
                <div className="p-4 sm:p-6 bg-indigo-900/20 rounded-[2rem] border border-indigo-500/20 shadow-inner">
                   <h5 className="text-xs sm:text-sm font-black text-indigo-400 uppercase mb-4 tracking-[0.2em] flex flex-wrap items-center gap-3">
                     <span className="text-xl">📜</span> Rules & Format
                   </h5>
                   <ul className="space-y-4 pl-1">
                      <li className="text-xs sm:text-sm font-medium text-slate-300 flex flex-wrap items-center gap-4">
                         <div className="w-6 h-6 rounded-full bg-indigo-500/20 border border-indigo-500/40 flex items-center justify-center text-xs sm:text-sm text-indigo-300 shadow-[0_0_10px_rgba(99,102,241,0.2)]">1</div>
                         Single elimination knockout format
                      </li>
                      <li className="text-xs sm:text-sm font-medium text-slate-300 flex flex-wrap items-center gap-4">
                         <div className="w-6 h-6 rounded-full bg-indigo-500/20 border border-indigo-500/40 flex items-center justify-center text-xs sm:text-sm text-indigo-300 shadow-[0_0_10px_rgba(99,102,241,0.2)]">2</div>
                         6 Over Super matches (Virtual)
                      </li>
                      <li className="text-xs sm:text-sm font-medium text-slate-300 flex flex-wrap items-center gap-4">
                         <div className="w-6 h-6 rounded-full bg-indigo-500/20 border border-indigo-500/40 flex items-center justify-center text-xs sm:text-sm text-indigo-300 shadow-[0_0_10px_rgba(99,102,241,0.2)]">3</div>
                         Grand Finals on Sunday, Jan 28
                      </li>
                   </ul>
                </div>
                <button className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-black py-5 rounded-[2rem] shadow-[0_0_20px_rgba(99,102,241,0.3)] uppercase text-xs sm:text-sm tracking-[0.2em] hover:shadow-[0_0_30px_rgba(99,102,241,0.5)] transition-all border border-purple-400/30">
                   Register My Team
                </button>
             </div>
          </div>
        ...
        {tab === 'standings' && (
           <div className="bg-[#161b22]/80 backdrop-blur-xl rounded-[3rem] p-4 sm:p-8 shadow-xl border border-[#30363d]">
              <div className="flex items-center justify-between mb-8 px-2">
                 <h4 className="text-xs sm:text-sm font-black text-purple-400 uppercase tracking-[0.2em] flex flex-wrap items-center gap-3">
                    <span className="text-xl">🏆</span> Team Rankings
                 </h4>
                 <span className="text-sm font-black text-slate-300 bg-white/10 px-4 py-1.5 rounded-full uppercase tracking-widest border border-white/5 backdrop-blur-md flex flex-wrap items-center gap-1">
              ...
              <div className="space-y-4">
                 {[
                   ...
                   <div key={idx} className="flex items-center justify-between p-5 bg-[#090d16]/80 rounded-[2rem] border border-[#30363d] shadow-inner hover:border-purple-500/30 transition-colors group">
                      <div className="flex flex-wrap items-center gap-4">
                         <div className={`w-10 h-10 rounded-2xl flex items-center justify-center text-xs sm:text-sm font-black text-white shadow-lg ${
                         ...
                         <div>
                            <p className="text-sm font-black text-white italic tracking-tighter group-hover:text-purple-300 transition-colors">{team.name}</p>
                            <p className="text-sm text-slate-500 font-bold uppercase tracking-widest mt-1">W: <span className="text-emerald-400">{team.w}</span> | L: <span className="text-rose-400">{team.l}</span></p>
                         </div>
                      </div>
