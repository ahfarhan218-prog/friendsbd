...
const Leaderboard: React.FC = () => {
  ...
  const fetchLeaderboard = async (p: string) => {
    setLoading(true);
    try {
      const r = await fetch(`${API_BASE}/leaderboard?period=${p}`);
  ...
  const getPointsKey = () => {
    if (period === 'daily') return 'dailyPoints';
    if (period === 'weekly') return 'weeklyPoints';
    return 'points';
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0a0a1a] via-[#0f0f2a] to-[#0a0a1a] pb-20">
      <style>{`
        ...
        .lb-card:hover { border-color:rgba(168,85,247,0.15); }
        .lb-input { background:rgba(22,27,34,0.8); border:1px solid #30363d; border-radius:12px; padding:10px 14px; color:#fff; font-size:0.85rem; outline:none; transition:all .3s; }
        .lb-input:focus { border-color:#a78bfa; }
        .lb-btn { padding:8px 20px; border-radius:12px; font-weight:700; font-size:0.75rem; border:none; cursor:pointer; transition:all .3s; }
        .lb-btn-primary { background:linear-gradient(135deg,#7c3aed,#a855f7); color:#fff; }
        .lb-btn-primary:hover { transform:translateY(-1px); box-shadow:0 8px 25px rgba(124,58,237,0.3); }
      ...
      <header className="bg-gradient-to-r from-purple-700 via-purple-600 to-pink-600 text-white p-4 sm:p-6 pb-16 sm:pb-20 rounded-b-[2rem] sm:rounded-b-[3rem] shadow-lg shadow-purple-900/30">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-black italic">Leaderboard 👑</h2>
              <p className="text-xs sm:text-sm opacity-70 font-bold uppercase tracking-wider">Top performers</p>
            </div>
          </div>
        </div>
      </header>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="lb-card p-4 sm:p-6 flex flex-col items-center">
              ...
              <p className="text-xs font-bold text-white/40 uppercase tracking-widest mb-4">{period === 'daily' ? "Today's" : period === 'weekly' ? "This Week's" : 'All Time'} Champions</p>
              <div className="flex items-end justify-center gap-3 sm:gap-6 w-full min-h-[200px]">
                ...
                {podium[1] && (
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="flex flex-col items-center flex-1 max-w-[120px]">
                    <div className="relative mb-2">
                      <img src={podium[1].avatar} className="w-14 h-14 sm:w-16 sm:h-16 rounded-full border-2 border-purple-400/30 object-cover shadow-lg" alt="" />
                      <span className="absolute -top-2 -right-2 text-xl">🥈</span>
                    </div>
                    <div className="w-full bg-gradient-to-t from-purple-700/40 to-purple-500/20 rounded-t-3xl h-20 sm:h-24 flex flex-col items-center justify-center p-2 text-center border-t border-purple-500/20">
                      <p className="text-xs font-black uppercase text-purple-300">2nd</p>
                      <p className="text-xs font-bold text-white truncate w-full px-1">{podium[1].name?.split(' ')[0]}</p>
                      <p className="text-xs font-medium text-purple-200">{podium[1][getPointsKey()] || 0} pts</p>
                    </div>
                  </motion.div>
                )}
                {/* 1st */}
                {podium[0] && (
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0 }} className="flex flex-col items-center flex-1 max-w-[140px]" style={{ animation: 'float 4s ease-in-out infinite' }}>
                    ...
                    <div className="w-full bg-gradient-to-t from-purple-600 to-purple-400/30 rounded-t-3xl h-28 sm:h-32 flex flex-col items-center justify-center p-2 text-center border-t border-purple-400/30 shadow-lg shadow-purple-900/20">
                      ...
                      <p className="text-xs sm:text-sm font-bold text-amber-200">{podium[0][getPointsKey()] || 0} pts</p>
                    </div>
                  </motion.div>
                )}
                {/* 3rd */}
                {podium[2] && (
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="flex flex-col items-center flex-1 max-w-[120px]">
                    <div className="relative mb-2">
                      <img src={podium[2].avatar} className="w-14 h-14 sm:w-16 sm:h-16 rounded-full border-2 border-purple-400/20 object-cover shadow-lg" alt="" />
                      <span className="absolute -top-2 -right-2 text-xl">🥉</span>
                    </div>
                    <div className="w-full bg-gradient-to-t from-purple-800/40 to-purple-600/20 rounded-t-3xl h-16 sm:h-20 flex flex-col items-center justify-center p-2 text-center border-t border-purple-500/10">
                      <p className="text-xs font-black uppercase text-purple-400">3rd</p>
                      <p className="text-xs font-bold text-white truncate w-full px-1">{podium[2].name?.split(' ')[0]}</p>
                      <p className="text-xs font-medium text-purple-300">{podium[2][getPointsKey()] || 0} pts</p>
                    </div>
                  </motion.div>
                )}
              </div>
            ...
            <div className="lb-card p-4">
              <div className="flex items-center gap-2 px-3 mb-4 text-purple-400 font-black uppercase text-xs tracking-widest">
                  ...
                  return (
                    <motion.div
                      ...
                      className={`flex items-center gap-3 p-3 rounded-2xl transition-all ${
                        isSelf
                          ? 'bg-purple-500/10 border border-purple-500/30 shadow-md'
                          : 'bg-[#161b22]/50 border border-transparent hover:bg-[#161b22] hover:border-white/5'
                      }`}
                    >
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center font-black text-xs ${
                        globalIndex <= 10 ? 'bg-purple-500/20 text-purple-400' : 'bg-[#30363d]/50 text-white/40'
                      }`}>
                        {globalIndex}
                      </div>
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="min-w-0">
                          <h4 className="font-bold text-sm text-white truncate">
                            {rank.name} {isSelf && <span className="text-purple-400 text-xs">(You)</span>}
                      ...
                      <div className="text-right shrink-0">
                        <p className="text-sm font-black text-purple-300">{pts.toLocaleString()}</p>
