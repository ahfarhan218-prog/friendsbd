...
const Profile: React.FC = () => {
  ...
  const targetUid = id || (session ? session.id : null);
  const isOwnProfile = session && (targetUid === session.id || targetUid === session.username);
  ...
  const saveAboutMe = async () => {
    ...
    try {
      ...
      if (s.id === profile.id) localStorage.setItem('user_session', JSON.stringify({ ...s, ...updates }));
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const newShout = { id: 'sh_' + Date.now(), displayId: Date.now(), user: profile.name, username: profile.username, userId: profile.id, avatar: imageUrl, content: `Updated their profile picture \n\n [img]${imageUrl}[/img]`, time: 'Just now', timestamp: Date.now(), userReactions: {}, replies: [], isPremium: !!profile.isPremium, isPinned: false, isClosed: false };
      await fetch(`${API_BASE}/shouts`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(newShout) });
      ...
      const s = JSON.parse(localStorage.getItem('user_session') || '{}');
      if (s.id === profile.id) localStorage.setItem('user_session', JSON.stringify({ ...s, avatar: imageUrl }));
    } catch (err: any) { }
  ...
  const saveBio = useCallback(async () => {
    ...
    await fetch(`${API_BASE}/users/${profile.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ bio: bioText }) });
    const s = JSON.parse(localStorage.getItem('user_session') || '{}');
    localStorage.setItem('user_session', JSON.stringify({ ...s, bio: bioText }));
    setEditingBio(false);
  }, [profile, bioText]);
  ...
  const updatePremiumSettings = useCallback(async (updates: any) => {
    ...
    await fetch(`${API_BASE}/users/${profile.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updates) });
    const s = JSON.parse(localStorage.getItem('user_session') || '{}');
    localStorage.setItem('user_session', JSON.stringify({ ...s, ...updates }));
  }, [profile]);
  ...
  const handleReaction = async (shoutId: string, reaction: string) => {
    ...
    try {
      ...
      if (res.ok) { const { userReactions } = await res.json(); setShouts(prev => prev.map(s => s.id === shoutId ? { ...s, userReactions } : s)); }
    } catch (err) { console.error(err); }
  };
  ...
  const progress = xpProgress(profile.points || 0, lvl);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0a0a1a] via-[#0f0f2a] to-[#0a0a1a] font-sans pb-20">
      <style>{`
        ...
        .pf-gradient-text { background:linear-gradient(135deg,#a78bfa,#ec4899); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-80 shrink-0">
            ...
            <div className="pf-card p-6 text-center">
              ...
              <h1 className="text-xl font-black text-white mt-4 flex items-center justify-center gap-2">
              ...
              <div className="inline-flex items-center gap-1.5 mt-2 px-3 py-1 rounded-full text-xs font-bold" style={{
                background: role === 'admin' ? 'rgba(244,63,94,0.15)' : role === 'moderator' ? 'rgba(249,115,22,0.15)' : 'rgba(124,58,237,0.12)',
                color: role === 'admin' ? '#fb7185' : role === 'moderator' ? '#fb923c' : '#a78bfa'
              }}>
                {role === 'admin' ? '🛡️' : role === 'moderator' ? '⚖️' : '👤'} {role.charAt(0).toUpperCase() + role.slice(1)}
              </div>

              {profile.customStatus && (
                <p className="text-xs text-purple-300/60 mt-2 flex items-center justify-center gap-1.5">
              ...
              <div className="grid grid-cols-2 gap-2 mt-5">
                {[
                  ...
                  { icon: '👥', label: 'Followers', value: profile.followers?.length || allUsers.length || 0, color: '#ec4899' },
              ...
              <div className="mt-5 pt-5 border-t border-[#30363d]">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {ACHIEVEMENTS.map(a => {
                    ...
                    return (
                      <div key={a.id} className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold transition-all ${unlocked ? 'bg-purple-500/10 text-purple-300' : 'bg-[#161b22]/50 text-white/20'}`}>
                ...
                userThreads.length === 0 ? <p className="text-white/30 text-sm text-center py-8">No topics created yet.</p>
                  : <div className="space-y-2 max-h-80 overflow-y-auto pr-1 custom-scrollbar">
                    {[...userThreads].sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0)).map(t => (
                      <Link key={t.id} to={`/forum/thread/${t.id}`} className="flex items-center gap-3 p-3 rounded-xl bg-[#161b22]/40 border border-[#30363d]/50 hover:bg-[#161b22] hover:border-purple-500/30 transition-all group">
                ...
                userPosts.length === 0 ? <p className="text-white/30 text-sm text-center py-8">No posts yet.</p>
                  : <div className="space-y-2 max-h-80 overflow-y-auto pr-1 custom-scrollbar">
                    {[...userPosts].sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0)).map(p => {
                      ...
                      return (
                        <Link key={p.id} to={`/forum/thread/${p.threadId}`} className="flex items-start gap-3 p-3 rounded-xl bg-[#161b22]/40 border border-[#30363d]/50 hover:bg-[#161b22] hover:border-purple-500/30 transition-all group">
                ...
                shouts.length === 0 ? <p className="text-white/30 text-sm text-center py-8">No shouts yet.</p>
                  : <div className="space-y-2 max-h-80 overflow-y-auto pr-1 custom-scrollbar">
                    {[...shouts].sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0)).map(s => {
                      ...
                      return (
                        <div key={s.id} className={`flex items-start gap-3 p-3 rounded-xl border transition-all ${s.isPinned
